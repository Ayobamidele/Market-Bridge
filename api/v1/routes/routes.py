...
<<<<<<< HEAD:api/v1/routes/routes.py
from api import app, db
from api.v1.forms.forms import LoginForm, RegistrationForm, ResetPasswordRequestForm, ResetPasswordForm, MessageForm, user_edit
from api.v1.models.models import User, Group, Role, Notification, Order
from api.v1.services.email import send_password_reset_email
from api.utils.tables import OrderTable
=======
...
>>>>>>> develop:supply_bridge/routes.py
...
def user():
    if request.method == 'POST':
        if request.json.get("create") == True:
            new_order = Order(title=f"Order-{random.randint(2383385933,2345678918376829)}",
                              contributors=[current_user],
                              content=f"{datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S')}"
                              )
            db.session.add(new_order)
            db.session.commit()
            return redirect(url_for('create_order', id=new_order.id, user=current_user.firstname))
    user_orders = current_user.orders
    print(current_user)
    html = """<img src="https://images.unsplash.com/photo-1596815064285-45ed8a9c0463?ixid=MXwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHw%3D&amp;ixlib=rb-1.2.1&amp;auto=format&amp;fit=crop&amp;w=1215&amp;q=80" alt="participant">"""
    user_order = ([{
        'title': o.title,
        "content": o.content,
    ...
    return render_template('user.html', user=current_user, emojis=emojis, orders=user_order)
...
def edit_user():
    ...
    if request.method == 'POST':
        if form.cancel.data:
            ...
            a = update_or_create(User,is_existed_keys=['id'],id=user.id,lastname=form.lastname.data,
                                firstname=form.firstname.data,
                                email=form.email.data, )
            db.session().commit()
            return redirect(url_for('user'))
    return render_template('user_edit.html', user=current_user, form=form, country=user.phone_country_code,emojis=emojis)
=======
@app.route("/profile", methods=["GET", "POST"])
@login_required
def user():
...
<<<<<<< HEAD:api/v1/routes/routes.py
    return render_template('notifications.html', title=f'notifications-{current_user.firstname}', notifications=user_notifications,
                           notification_display="none",user=current_user, emojis=emojis)
=======
    return render_template(
        "notifications.html",
...
<<<<<<< HEAD:api/v1/routes/routes.py
    ...
    return render_template('notification.html', title=f"notifications-{user_notification['name']}", notification=user_notification,
                           user=current_user, emojis=emojis)
@app.route('/order/<user>/<id>')
@login_required
def create_order(id,user):
    order = Order.query.filter_by(id=id).first()
    return render_template('create_list.html', order=order)
