...
import phonenumbers
<<<<<<< HEAD:api/v1/forms/forms.py
from api.v1.models import User
=======
from .models import User, Measure
from flask_login import current_user
from phonenumbers import geocoder


# from flask_login import login_required, current_user
>>>>>>> develop:supply_bridge/forms.py


class MeasureForm(FlaskForm):
	name = StringField('measure',
							validators=[DataRequired(), Length(min=2, max=20)])
	
	# create = SubmitField("Yay! Create",
	# 	    			render_kw={'id':'submit-measure-input' ,'class':'btn btn-success'})

	# cancel = SubmitField('Oops! Cancel',
	# 					 render_kw={'form-novalidate': True, 'class': 'btn btn-warning', 'for':'my-modal-6'})
	...
	def validate_name(self, name):
		...
		if measure is not None:
			raise ValidationError('Please that option is available.')
